...
export default function ScreeningToolContent() {
  ...
  return (
    <div className="min-h-screen bg-[#faf7f2] dark:bg-[#171412] text-[#211a13] dark:text-[#ece5da]">
      <WarmNav active="tool" />

      <div className="px-6 pb-10 pt-16 text-center md:px-16 md:pt-20">
        <div className="mb-4.5 text-[13px] font-semibold uppercase tracking-wide text-[#c2410c] dark:text-[#fb923c]">
        ...
        <p className="mx-auto max-w-2xl text-lg leading-relaxed text-[#5d554b] dark:text-[#a89f92] md:text-xl">
      ...
      <div className="grid gap-6 px-6 pb-16 pt-6 md:grid-cols-2 md:px-16 md:pb-20">
        {/* Input card */}
        <div className="rounded-2xl border border-[#e8e1d6] dark:border-[#37322b] bg-white dark:bg-[#201c18] p-8">
            ...
            {questionBank.map((q, i) => (
              <button
                ...
                className={`rounded-full border-[1.5px] px-3.5 py-2 text-[13px] font-semibold ${
                  i === selected ? 'border-[#211a13] bg-[#211a13] text-white dark:border-[#ece5da] dark:bg-[#ece5da] dark:text-[#211a13]' : 'border-[#d8d0c3] dark:border-[#4a443b] bg-white dark:bg-[#201c18] text-[#5d554b] dark:text-[#a89f92]'
                }`}
              >
                {q.short}
              </button>
          ...
          <div className="mb-5 min-h-[72px] rounded-xl border border-[#e8e1d6] dark:border-[#37322b] bg-[#faf7f2] dark:bg-[#171412] px-5 py-4.5 text-base leading-snug">
          ...
          <textarea
            ...
            className="mb-5 w-full resize-y rounded-lg border border-[#d8d0c3] dark:border-[#4a443b] bg-[#faf7f2] dark:bg-[#171412] px-4 py-3.5 text-[15px] text-[#211a13] dark:text-[#ece5da] outline-none"
          />
          <button
            onClick={generate}
        ...
        <div className="flex min-h-[420px] flex-col gap-4">
          {idle && (
            <div className="flex flex-1 flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-[#d8d0c3] dark:border-[#4a443b] p-12 text-center">
          ...
          {loading && (
            <div className="flex flex-1 items-center justify-center gap-3.5 rounded-2xl border border-[#e8e1d6] dark:border-[#37322b] bg-white dark:bg-[#201c18] text-base text-[#8a8177]">
          ...
          {status === 'error' && (
            <div className="flex flex-1 items-center justify-center rounded-2xl border border-[#e8e1d6] dark:border-[#37322b] bg-white dark:bg-[#201c18] p-8 text-center text-[#9a3412]">
              {error}
            </div>
          )}
          {done && (
            <div className="flex animate-[srg-in_0.4s_ease] flex-col gap-4">
              {drafts.map((d) => (
                <div key={d.angle} className="rounded-2xl border border-[#e8e1d6] dark:border-[#37322b] bg-white dark:bg-[#201c18] p-7">
                  <div className="mb-3 flex items-center justify-between">
                    <span className="text-xs font-bold uppercase tracking-wide text-[#c2410c] dark:text-[#fb923c]">{d.angle}</span>
                    <button
                      onClick={() => navigator.clipboard.writeText(d.text)}
                      className="rounded-full border-[1.5px] border-[#d8d0c3] dark:border-[#4a443b] px-3.5 py-1.5 text-[13px] font-semibold text-[#5d554b] dark:text-[#a89f92]"
                    >
                      Copy
                    </button>
                  </div>
              ...
              <div className="rounded-xl bg-[#f3ede3] px-4.5 py-3.5 text-sm text-[#5d554b] dark:text-[#a89f92]">
                <strong className="text-[#c2410c] dark:text-[#fb923c]">Final step is human:</strong> I pick the angle, sharpen it, and
      ...
      {/* How it works */}
      <div className="grid gap-6 border-t border-[#e8e1d6] dark:border-[#37322b] px-6 py-14 md:grid-cols-3 md:px-16">
        {howItWorks.map((s) => (
          <div key={s.n}>
            <div className="mb-2 text-3xl text-[#c2410c] dark:text-[#fb923c]" style={serif}>
              {s.n}
            </div>
            <div className="mb-1.5 text-base font-bold">{s.title}</div>
            <div className="text-[15px] leading-snug text-[#5d554b] dark:text-[#a89f92]">{s.body}</div>
          </div>
        ))}
      </div>
      ...
      {/* Footer */}
      <footer className="flex flex-col gap-3 border-t border-[#e8e1d6] dark:border-[#37322b] px-6 py-6.5 text-sm text-[#8a8177] md:flex-row md:items-center md:justify-between md:px-16">
        <Link href="/" className="hover:text-[#211a13] dark:hover:text-[#ece5da]">
          ← Back to home
        </Link>
